...
w1{3} = rand(n_hd3,n_hd2+1);

dw0{3}= zeros(n_hd3,n_hd2+1); %rand(n_ou,n_hd)./2  - 0.25;%

w1{4} = rand(n_hd4,n_hd3+1);

dw0{4}= zeros(n_hd4,n_hd3+1); %rand(n_ou,n_hd)./2  - 0.25;%

w1{5} = rand(n_ou,n_hd4+1);

dw0{5}= zeros(n_ou,n_hd4+1); %rand(n_ou,n_hd)./2  - 0.25;%
...
while mse_train > mse_thres && epoch <= num_Epoch
    ...
    for i = 1:50,
        ...
        hd1 = [hyperb(w1{1}*x);1];          % hidden neurons are nonlinear
        %hd1 = hd1';
        %a =1;
        hd2 = [hyperb(w1{2}*hd1);1]; %%%%%%second layer
        
        hd3 = [hyperb(w1{3}*hd2);1];
        
        hd4 = [hyperb(w1{4}*hd3);1];
        ...
        delta_ou = e(:,i).*d_hyperb(w1{5}*hd4);
        delta_hd4 = d_hyperb(w1{4}*hd3).*(w1{5}(:,1:n_hd4)'*delta_ou);
        delta_hd3 = d_hyperb(w1{3}*hd2).*(w1{4}(:,1:n_hd3)'*delta_hd4);
        delta_hd2 = d_hyperb(w1{2}*hd1).*(w1{3}(:,1:n_hd2)'*delta_hd3);
        %delta_hd1 = d_hyperb(w1{1}*x).*(w1{2}(:,1:n_hd1)'*delta_hd2);
        delta_hd1 = d_hyperb(w1{1}*x).*(w1{2}(:,1:n_hd1)'*delta_hd2);

        dw1{1} = eta1(epoch)*delta_hd1*x';
        dw1{2} = eta1(epoch)*delta_hd2*hd1';
        dw1{3} = eta1(epoch)*delta_hd3*hd2';
        dw1{4} = eta1(epoch)*delta_hd4*hd3';
        dw1{5} = eta1(epoch)*delta_ou*hd4';
        ...
        %% weights update
        %w2{1} = w1{1} + alpha*dw0{1} + （1-alpha）dw1{1};  % weights input -> hidden
        ...
        w2{4} = w1{4} + alpha*dw0{4} + dw1{4};
        w2{5} = w1{5} + alpha*dw0{5} + dw1{5};
